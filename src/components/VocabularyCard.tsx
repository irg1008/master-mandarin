      ...
      <div className={`card-inner w-full h-full cursor-pointer ${isFlipped ? "flipped" : ""}`}>
        ...
        >
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-1">
            ...
            <button
              onClick={handleAudio}
              className={`mt-2 p-1.5 rounded-full bg-secondary/10 hover:bg-secondary/20 transition-colors cursor-pointer ${isPlaying ? "animate-pulse" : ""}`}
              aria-label={`Play pronunciation for ${entry.hanzi}`}
            >
              <Volume2 className="w-3 h-3 md:w-4 md:h-4 text-secondary" />
            </button>
          </div>
